...
magnitudes = [[0 for _ in range(len(input))] for _ in range(len(input))]

for i in range(len(input)):
    flattened = []
    flatten(input[i], 1, flattened)
    for j in range(len(input)):
        if i == j:
            continue
        flattenedNumber = [] 
        flatten(input[j], 1, flattenedNumber)
        addition = add(flattened, flattenedNumber)
        addition = reduce(addition)
        magnitudes[i][j] = magnitude(addition)

print(max([max(x) for x in magnitudes]))